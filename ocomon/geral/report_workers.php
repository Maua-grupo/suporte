...
<body class="app-embedded-screen">
    ...
    <div class="container">
        <h5 class="my-4"><i class="fas fa-user-md text-secondary"></i>&nbsp;<?= TRANS('TTL_OPERATOR_AREA_ATTEND'); ?></h5>
        ...
        ?>
            ...
            $query = "SELECT count(o.numero) quantidade, u.nome operador, s.sistema area 
                FROM 
                    usuarios as u, sistemas as s, `status` st,
                    ocorrencias as o LEFT JOIN tickets_extended te ON te.ticket = o.numero
                WHERE 
                    o.operador = u.user_id AND 
                    u.nivel <> 3 AND -- não exibe os usuários somente-abertura
                    o.status = st.stat_id AND 
                    st.stat_ignored <> 1 AND 
                    u.AREA = s.sis_id AND
                    te.main_worker IS NULL
                    ";

            /* Consulta na ticket_x_workers */
            $queryWorkers = "SELECT 
                                COUNT(o.numero) quantidade, 
                                u.nome operador, 
                                s.sistema area
                            FROM 
                                usuarios as u, 
            ...
            if (!empty($filter_areas)) {
                ...
                if (!empty($_POST['area']) && ($_POST['area'] != -1)) {
                    ...
                    $queryWorkers .= " AND o.sistema = " . $_POST['area'] . "";
                    ...
                    $queryWorkers .= " AND o.sistema IN ({$u_areas}) ";
            ...
            if ((!isset($_POST['d_ini'])) || (!isset($_POST['d_fim']))) {
                ...
                if ($d_ini <= $d_fim) {
                    ...
                    $query .= " {$extraTerms} {$clausule}
                        GROUP BY u.nome, s.sistema ORDER BY quantidade desc, u.nome";
                    $queryWorkers .= " {$extraTerms} {$clausule}
                        GROUP BY u.nome, s.sistema ORDER BY quantidade desc, u.nome";

                    $resultado = $conn->query($query);
                    $resultadoWorkers = $conn->query($queryWorkers);


                    $array1 = $resultado->fetchAll();
                    $array2 = $resultadoWorkers->fetchAll();
                    ...
                    if (count($array1) && count($array2)) {
                        $mergedArray = array_values(
                            array_reduce(
                                $array2,
                                function($result, $row) {
                                    if (!isset($result[$row['operador']])) {
                                        $result[$row['operador']] = $row;
                                    } else {
                                        $result[$row['operador']]['quantidade'] += $row['quantidade'];
                                        // $result[$row['operador']]['rows'] += $row['rows'];
                                    }
                                    return $result;
                                },
                                array_column($array1, null, 'operador')
                            )
                        );
                    } elseif (count($array1)) {
                        $mergedArray = $array1;
                    ...
                    if (!empty($mergedArray)) {
                        $mergedArray = arraySortByColumn($mergedArray, 'quantidade', SORT_DESC);
                    }
                            ...
                            <table class="table table-striped table-bordered">
                                ...
                                <thead>
                                    <tr class="header table-borderless">
                                        <td class="line"><?= mb_strtoupper(TRANS('TECHNICIAN')); ?></td>
                                        <td class="line"><?= mb_strtoupper(TRANS('OPERATOR_AREA')); ?></td>
    ...
    <script type='text/javascript'>
        ...
        function showChart(canvasID) {
            ...
            for (var i in dataFromPHP) {
                ...
                labels.push(dataFromPHP[i].operador + '(' + dataFromPHP[i].area + ')');
            ...
            var myChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: '<?= TRANS('TTL_OPERATOR_AREA_ATTEND','',1); ?>',
                    },
                    scales: {
                        yAxes: [{
                            display: false,
