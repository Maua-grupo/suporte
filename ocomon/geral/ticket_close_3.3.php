...
if (!isset($_POST['numero']) && isset($_GET['numero'])) {
    ...
    $sqlSoluc = "SELECT * FROM solucoes WHERE numero = " . $numero . " ";
    $execSoluc = $conn->query($sqlSoluc);
    $regSoluc = $execSoluc->rowCount();
    /* JÁ ENCERRADA NO SISTEMA */
    if ($regSoluc > 0) {

        echo message('danger', 'Ooops!', TRANS('MSG_ALERT_OCCO_IS_LOCKED_UP'), '', 'ticket_show.php?numero=' . $numero);
        exit;
    }

    /* CHECAGEM DE DEPENDÊNCIAS */
    $sqlSub = "select * from ocodeps where dep_pai = " . $numero . " ";
    $execSub = $conn->query($sqlSub);
    $deps = array();
    foreach ($execSub->fetchAll() as $rowSub) {
    ...
    if (sizeof($deps)) {
        ...
        foreach ($deps as $err) {
            $saida .= "Chamado <a onClick=\"popup_alerta('ticket_show.php?popup=true&numero=" . $err . "')\"><font color='blue'>" . $err . "</font></a><br>";
        }
        $saida .= "<br><a align='center' onClick=\"redirect('ticket_show.php?numero=" . $numero . "');\">&nbsp;" . TRANS('TXT_RETURN') . "</a>";
        // echo $saida;
        $voltar = "ticket_show.php?numero=" . $numero;
        echo message('warning', 'Ooops!', $saida, '', '', true);
        exit;
    }
}
...
if (!isset($_POST['submit'])) {
    ...
    /* CHECAGEM PARA VER SE É CHAMADO PAI */
    // $qrySubCall = "select * from ocodeps where dep_pai = " . $numero . "";
    ...
    <div class="container-fluid">
        ...
        <form name="form" id="form" method="post" action="<?= $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
            ...
            <div class="form-group row my-4">
                ...
                <div class="form-group col-md-4">
                    <select class="form-control " id="idArea" name="sistema" required onChange="ajaxFunction('divSelProblema', 'showSelProbs.php', 'idLoad', 'prob=idProblema', 'area_cod=idArea', 'area_habilitada=idAreaHabilitada'); ajaxFunction('divProblema', 'showProbs.php', 'idLoad', 'prob=idProblema', 'area_cod=idArea');">
                        ...
                        $query = "SELECT s.sis_id, s.sistema from sistemas s, {$table} a WHERE s.sis_status NOT IN (0) " .
                            ...
                            "GROUP BY sis_id, sistema ORDER BY sistema";
                ...
                <div class="form-group col-md-4">
                    <select class="form-control  " id="idStatus" name="status" disabled="disabled">
                ...
                <input type='hidden' name='antes' value='<?= $row['status']; ?>' />
