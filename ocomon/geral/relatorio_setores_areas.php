...
<body class="app-embedded-screen">
    ...
    <div class="container">
        <h5 class="my-4"><i class="fas fa-building text-secondary"></i>&nbsp;<?= TRANS('DEPARTMENTS_X_AREAS'); ?></h5>
        ...
        ?>
            ...
            $query = "SELECT count(*) AS quantidade, l.local AS setor, s.sistema AS area 
                        FROM ocorrencias AS o, localizacao AS l, sistemas AS s, status st
						WHERE 
                            o.status = st.stat_id AND 
                            st.stat_ignored <> 1 AND 
                            o.sistema = s.sis_id AND o.local = l.loc_id ";
            
            $queryRules = " FROM ocorrencias AS o, localizacao AS l, sistemas AS s, status st 
            ...
            if ((!isset($_POST['d_ini'])) || (!isset($_POST['d_fim']))) {
                ...
                if ($d_ini <= $d_fim) {
                    ...
                    $queryChart = "SELECT count(*) AS quantidade, l.local AS setor ";
                    $queryChart .= $queryRules . " AND o.data_fechamento >= '".$d_ini."' AND o.data_fechamento <= '".$d_fim."' AND
                    o.data_atendimento IS NOT NULL 
                    {$clausule}
                    GROUP  BY l.local ORDER BY setor, quantidade desc";
                    ...
                    $queryChart2 .= $queryRules . " AND o.data_fechamento >= '".$d_ini."' AND o.data_fechamento <= '".$d_fim."' AND
                    o.data_atendimento IS NOT NULL 
                    {$clausule}
                    ...
                    $query .= " AND o.data_fechamento >= '".$d_ini."' AND o.data_fechamento <= '".$d_fim."' AND
					            o.data_atendimento IS NOT NULL 
                                {$clausule}
					            GROUP  BY l.local, s.sistema ORDER BY setor, quantidade desc, area"; /* area, quantidade desc, setor */
                    ...
                    if ($linhas == 0) {
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered">
                                <!-- table-hover -->
                                <!-- <caption><?= TRANS('DEPARTMENTS_X_AREAS'); ?></caption> -->
    ...
    <script type='text/javascript'>
        ...
        function showChart(canvasID) {
            ...
            for (var i in dataFromPHP) {
                ...
                labels.push(dataFromPHP[i].setor);
