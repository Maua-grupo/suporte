...
<body>
    ...
    <div class="container">
        <h5 class="my-4"><i class="fas fa-exclamation-circle text-secondary"></i>&nbsp;<?= TRANS('TLT_PROBLEMS_AREAS'); ?></h5>
        ...
        ?>
            ...
            $query = "SELECT count(*)  AS quantidade, s.sistema AS area,  p.problema as problema
					FROM 
                        ocorrencias AS o, localizacao AS l, sistemas AS s, problemas as p, status as st
					WHERE 
                    o.status = st.stat_id AND st.stat_ignored <> 1 AND 
                    o.sistema = s.sis_id AND o.local = l.loc_id and o.problema = p.prob_id ";
            
            $queryRules = " FROM ocorrencias AS o, localizacao AS l, sistemas AS s, problemas as p, status as st 
					WHERE o.status = st.stat_id AND st.stat_ignored <> 1 AND o.sistema = s.sis_id AND o.local = l.loc_id and o.problema = p.prob_id ";
            ...
            if ((!isset($_POST['d_ini'])) || (!isset($_POST['d_fim']))) {
                ...
                if ($d_ini <= $d_fim) {
                    ...
                    $queryChart = "SELECT count(*)  AS quantidade, p.problema as problema ";
                    $queryChart .= $queryRules . " AND o.data_fechamento >= '" . $d_ini . "' AND o.data_fechamento <= '" . $d_fim . "' AND o.data_atendimento IS NOT NULL 
                    {$clausule}
                    GROUP  BY p.problema ORDER BY quantidade desc, problema";
                    ...
                    $queryChart2 = "SELECT count(*)  AS quantidade, s.sistema AS area ";
                    $queryChart2 .= $queryRules . " AND o.data_fechamento >= '" . $d_ini . "' AND o.data_fechamento <= '" . $d_fim . "' AND o.data_atendimento IS NOT NULL 
                    {$clausule}
                    GROUP  BY s.sistema ORDER BY quantidade desc";
                    $resultadoChart2 = $conn->query($queryChart2);

                    $query .= " AND o.data_fechamento >= '" . $d_ini . "' AND o.data_fechamento <= '" . $d_fim . "' AND
						            o.data_atendimento IS NOT NULL 
                                    {$clausule}
							        GROUP  BY p.problema, s.sistema ORDER BY problema, quantidade desc";
    ...
    <script type='text/javascript'>
        ...
        function showChart(canvasID) {
            ...
            for (var i in dataFromPHP) {
                ...
                labels.push(dataFromPHP[i].problema);
            ...
            var myChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    title: {
                        display: true,
                        text: '<?= TRANS('TICKETS_BY_TYPE_OF_PROBLEM','',1); ?>',
                    },
                    scales: {
                        yAxes: [{
                            display: false,
