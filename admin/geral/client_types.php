...
<body>
	...
	<div class="container-fluid">
		...
		$types = (isset($_GET['cod']) ? getClientsTypes($conn, $_GET['cod']) : getClientsTypes($conn));
		...
		?>
			...
			?>
				<table id="table_lists" class="stripe hover order-column row-border" border="0" cellspacing="0" width="100%">
					...
					<thead>
						<tr class="header">
							<td class="line issue_type"><?= TRANS('CLIENT_TYPE'); ?></td>
					...
					<tbody>
						...
						foreach ($types as $row) {
						    ?>
							<tr>
								<td class="line"><?= $row['type_name']; ?></td>
								<td class="line"><?= $row['type_description']; ?></td>
		...
		if ((isset($_GET['action'])  && ($_GET['action'] == "new")) && !isset($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				<?= csrf_input('client_types'); ?>
				<div class="form-group row my-4">
					<label for="client_type" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('CLIENT_TYPE'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="client_type" name="client_type" required />
                    </div>

                    <label for="type_description" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('DESCRIPTION'); ?></label>
					<div class="form-group col-md-10">
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && empty($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="client_type" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('CLIENT_TYPE'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="client_type" name="client_type" value="<?= $row['type_name']; ?>" required />
						
                    </div>
                    
                    <label for="type_description" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('DESCRIPTION'); ?></label>
					<div class="form-group col-md-10">
						<textarea class="form-control" id="type_description" name="type_description" rows="3"><?= $row['type_description']; ?></textarea>
                    </div>
