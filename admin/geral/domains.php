...
<body class="app-embedded-screen">
	...
	<div class="container-fluid">
		<h4 class="my-4"><i class="fas fa-network-wired text-secondary"></i>&nbsp;<?= TRANS('NET_DOMAINS'); ?></h4>
		...
		$domains = getDomains($conn);

		if (isset($_GET['cod'])) {
			// $query .= " WHERE dom_cod = ".(int)$_GET['cod']."  ";
			$domains = getDomains($conn, (int)$_GET['cod']);
		}
        // $query .= " ORDER BY dom_desc";
		// $resultado = $conn->query($query);
		// $registros = $resultado->rowCount();
		$registros = count($domains);
		...
		?>
			...
			?>
				<table id="table_lists" class="stripe hover order-column row-border" border="0" cellspacing="0" width="100%">
					...
					<thead>
						<tr class="header">
							<td class="line issue_type"><?= TRANS('NET_DOMAIN'); ?></td>
					...
					<tbody>
						...
						?>
							<tr>
								<td class="line"><?= $row['dom_desc']; ?></td>
								<td class="line"><?= $row['nickname']; ?></td>
								<td class="line"><?= $row['inst_nome']; ?></td>
								<td class="line"><button type="button" class="btn btn-secondary btn-sm" onclick="redirect('<?= $_SERVER['PHP_SELF']; ?>?action=edit&cod=<?= $row['dom_cod']; ?>')"><?= TRANS('BT_EDIT'); ?></button></td>
								<td class="line"><button type="button" class="btn btn-danger btn-sm" onclick="confirmDeleteModal('<?= $row['dom_cod']; ?>')"><?= TRANS('REMOVE'); ?></button></td>
							</tr>
		...
		if ((isset($_GET['action'])  && ($_GET['action'] == "new")) && !isset($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="domain_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('NET_DOMAIN'); ?></label>
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && empty($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="domain_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('NET_DOMAIN'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="domain_name" name="domain_name" value="<?= $row['dom_desc']; ?>" required />
                    </div>
