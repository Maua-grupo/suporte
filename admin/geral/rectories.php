...
<body>
	...
	<div class="container-fluid">
		<h4 class="my-4"><i class="fas fa-university text-secondary"></i>&nbsp;<?= TRANS('RECTORIES'); ?></h4>
		...
		// $query = "SELECT * FROM reitorias ";
		$rectories = getRectories($conn);

		if (isset($_GET['cod'])) {
			// $query .= " WHERE reit_cod = ".(int)$_GET['cod']."  ";
			$rectories = getRectories($conn, (int)$_GET['cod']);
		}
        // $query .= " ORDER BY reit_nome";
		// $resultado = $conn->query($query);
		// $registros = $resultado->rowCount();
		$registros = count($rectories);
		...
		?>
			...
			?>
				<table id="table_lists" class="stripe hover order-column row-border" border="0" cellspacing="0" width="100%">
					...
					<thead>
						<tr class="header">
							<td class="line issue_type"><?= TRANS('COL_RECTORY'); ?></td>
					...
					<tbody>
						...
						?>
							<tr>
								...
								<td class="line"><button type="button" class="btn btn-secondary btn-sm" onclick="redirect('<?= $_SERVER['PHP_SELF']; ?>?action=edit&cod=<?= $row['reit_cod']; ?>')"><?= TRANS('BT_EDIT'); ?></button></td>
								<td class="line"><button type="button" class="btn btn-danger btn-sm" onclick="confirmDeleteModal('<?= $row['reit_cod']; ?>')"><?= TRANS('REMOVE'); ?></button></td>
							</tr>
		...
		if ((isset($_GET['action'])  && ($_GET['action'] == "new")) && !isset($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="rectory_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('COL_RECTORY'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="rectory_name" name="rectory_name" required />
                    </div>
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && empty($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="rectory_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('COL_RECTORY'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="rectory_name" name="rectory_name" value="<?= $row['reit_nome']; ?>" required />
                    </div>
