...
<body class="app-embedded-screen">
	...
	<div class="container-fluid">
		<h4 class="my-4"><i class="fas fa-hashtag text-secondary"></i>&nbsp;<?= TRANS('CLIENT_STATUS'); ?></h4>
		...
		$status = (isset($_GET['cod']) ? getClientsStatus($conn, $_GET['cod']) : getClientsStatus($conn));
		...
		?>
			...
			?>
				<table id="table_lists" class="stripe hover order-column row-border" border="0" cellspacing="0" width="100%">
					...
					<thead>
						<tr class="header">
							<td class="line issue_type"><?= TRANS('CLIENT_STATUS'); ?></td>
					...
					<tbody>
						...
						foreach ($status as $row) {
						    ?>
							<tr>
								<td class="line"><?= $row['status_name']; ?></td>
								<td class="line"><?= $row['status_description']; ?></td>
		...
		if ((isset($_GET['action'])  && ($_GET['action'] == "new")) && !isset($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				<?= csrf_input('client_status'); ?>
				<div class="form-group row my-4">
					<label for="status_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('CLIENT_STATUS'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="status_name" name="status_name" required />
                    </div>

                    <label for="status_description" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('DESCRIPTION'); ?></label>
					<div class="form-group col-md-10">
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && empty($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="status_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('CLIENT_STATUS'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="status_name" name="status_name" value="<?= $row['status_name']; ?>" required />
						
                    </div>
                    
                    <label for="status_description" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('DESCRIPTION'); ?></label>
					<div class="form-group col-md-10">
						<textarea class="form-control" id="status_description" name="status_description" rows="3"><?= $row['status_description']; ?></textarea>
                    </div>
