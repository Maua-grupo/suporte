...
<body class="app-embedded-screen">
	...
	<div class="container-fluid">
		<h4 class="my-4"><i class="fas fa-photo-video text-secondary"></i>&nbsp;<?= TRANS('SW_CATEGORIES'); ?></h4>
		...
		if (isset($_GET['cod'])) {
			$query .= " WHERE cat_cod = ".(int)$_GET['cod']."  ";
		}
					...
					<tbody>
						...
						?>
							<tr>
								...
								<td class="line"><button type="button" class="btn btn-secondary btn-sm" onclick="redirect('<?= $_SERVER['PHP_SELF']; ?>?action=edit&cod=<?= $row['cat_cod']; ?>')"><?= TRANS('BT_EDIT'); ?></button></td>
								<td class="line"><button type="button" class="btn btn-danger btn-sm" onclick="confirmDeleteModal('<?= $row['cat_cod']; ?>')"><?= TRANS('REMOVE'); ?></button></td>
							</tr>
		...
		if ((isset($_GET['action'])  && ($_GET['action'] == "new")) && !isset($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="categorie_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('COL_NAME'); ?></label>
		...
		if ((isset($_GET['action']) && $_GET['action'] == "edit") && empty($_POST['submit'])) {
			...
			<form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
				...
				<div class="form-group row my-4">
					<label for="categorie_name" class="col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('COL_NAME'); ?></label>
					<div class="form-group col-md-10">
						<input type="text" class="form-control " id="categorie_name" name="categorie_name" value="<?= $row['cat_desc']; ?>" required />
                    </div>
