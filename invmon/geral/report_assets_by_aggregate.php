...
<body>
    ...
    <div class="container">
        <h5 class="my-4"><i class="fas fa-puzzle-piece text-secondary"></i>&nbsp;<?= TRANS('ASSETS_BY_AGGREGATE'); ?></h5>
        ...
        ?>
            <form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" id="form">
                <div class="form-group row my-4">
                    ...
                    <label for="asset_aggregate" class="col-sm-2 col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('COMPONENT'); ?></label>
                    <div class="form-group col-md-10">
                        <select class="form-control bs-select" id="asset_aggregate" name="asset_aggregate">
                            <?php
                    ...
                    <label for="component_attribute" class="col-sm-2 col-md-2 col-form-label col-form-label-sm text-md-right"><?= TRANS('COMPONENT_ATTRIBUTE'); ?></label>
                    <div class="form-group col-md-10">
                        <select class="form-control bs-select" id="component_attribute" name="component_attribute">
                            <?php
    ...
    <script type='text/javascript'>
        $(function() {
            ...
            $('#idSubmit').on('click', function() {
                ...
                $.ajax({
					url: './report_assets_by_aggregate_process.php',
					method: 'POST',
					data: $('#form').serialize(),
					dataType: 'json',
				}).done(function(data) {
					...
					if (!data.success) {
                        ...
                        table += '<thead>';
                            table += '<tr class="header table-borderless">';
                                table += '';
                                table += '<td class="line"><?= mb_strtoupper(TRANS('ATTRIBUTE')); ?></td>';
                        ...
                        for (var i in data['table']) {
                            ...
                            if (data['table'][i].asset_type !== undefined) {
                                table += '<tr>';
                                table += '<td class="line">' + data['table'][i].measure + '</td>';
                        ...
                        const chart_01 = report_assets_general(data, 'chart_01', 'measure', 'graph_01');
